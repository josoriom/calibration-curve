...
function xyReduce(data, options = {}) {
    ...
    const { x, y } = data;
    const { from = x[0], to = x[x.length - 1], nbPoints = 4001, optimize = false, } = options;
    let { zones = [] } = options;
    ...
    for (const zone of internalZones) {
        zone.fromIndex = (0, x_1.xFindClosestIndex)(x, zone.from);
        zone.toIndex = (0, x_1.xFindClosestIndex)(x, zone.to);
